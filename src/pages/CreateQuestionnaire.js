...
const CreateQuestionnaire = () => {
  ...
  const [nodes, setNodes] = useState([
    {
      key: 'root',
      label: 'Questionnaire',
      icon: 'fa-duotone fa-hashtag fa-xl',
      type: 'Group',
      data: {
        label: 'Questionnaire',
        id: 'group',
  ...
  const items = [
    ...
    {
      label: 'Group',
      icon: 'fa-duotone fa-layer-group green-group',
      command: () => {
  ...
  const accept = () => {
    ...
    const ns = new NodeService();
    ns.postQuestionnaire(nodes[0]).then(
      (data) => {
        // Message
  ...
  const acceptDownload = () => {
    console.log(nodes);
    const ns = new NodeService();
    ns.getQuestionnaireXLSForm('f87b1a1a-7786-4b20-9052-df1ecf53015b').then(
      (data) => {
        // Message
        let element = document.createElement('a');
        element.setAttribute('href',
          `data:text/plain;charset=utf-8,${encodeURIComponent(data.data)}`);
